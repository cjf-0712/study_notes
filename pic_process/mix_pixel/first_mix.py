    ...
    try:
        ...
        for y in range(0, height, 4):
            for x in range(0, width, 4):
                ...
                # 定义函数来计算平均值
                def average_channel(group, channel):
                    return int(sum([pixel[channel] for pixel in group]) / len(group))
                ...
                r_avg = average_channel(group1, 0)
                pixels[x + 0, y + 1] = (r_avg, 0, 0)

                # 处理 (1,0): G数据
                g_avg = average_channel(group1, 1)
                pixels[x + 1, y + 0] = (0, g_avg, 0)

                # 处理 (1,1): B数据
                b_avg = average_channel(group1, 2)
                ...
                g_avg2 = average_channel(group2, 1)
                pixels[x + 0, y + 2] = (0, g_avg2, 0)

                # 处理 (0,3): B数据
                b_avg2 = average_channel(group2, 2)
                pixels[x + 0, y + 3] = (0, 0, b_avg2)

                # 处理 (1,3): R数据
                r_avg2 = average_channel(group2, 0)
                ...
                r_avg3 = average_channel(group3, 0)
                pixels[x + 2, y + 0] = (r_avg3, 0, 0)

                # 处理 (3,0): b数据
                group3 = [block[2][0], block[2][1], block[3][0], block[3][1]]
                b_avg3 = average_channel(group3, 2)
                pixels[x + 3, y + 0] = (0, 0, b_avg3)

                # 处理 (3,1): G数据
                g_avg3 = average_channel(group3, 1)
                ...
                b_avg4 = average_channel(group4, 2)
                pixels[x + 2, y + 2] = (0, 0, b_avg4)

                # 处理 (2,3): G数据
                g_avg4 = average_channel(group4, 1)
                pixels[x + 2, y + 3] = (0, g_avg4, 0)

                # 处理 (3,2): R数据
                r_avg4 = average_channel(group4, 0)
...
if __name__ == "__main__":
    input_directory = r"F:\FPGA\unilumin_work\pic\GEN_COLOR"
    output_directory = r"F:\FPGA\unilumin_work\pic\first_1"
