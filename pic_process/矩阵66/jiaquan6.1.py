...
#quexian_moni
##IDE2:在整合亮度设置和模拟图像缺陷和缩放功能，整合了个图像处理前置模块
    ...
    try:
        ...
        for y in range(0, height, 4):
            for x in range(0, width, 4):
                ...
                # 检查边界条件
                lef_bd = (x == 0)  # 是否在左边界
                bot_bd = (y == height - 1 )  # 是否在下边界末行
                bot_bd2 = (y == height - 2)   # 是否在下边界次行
                ...
                # 处理 (0,1): G数据
                #边界情况的算式
                group1 = [block[-1][0], block[0][-1], block[1][-1], block[0][0], block[1][0], block[1][1]]
                weight_g1 = [0.25, 0.25, 0.25, 0.25, 0.25, 0.25]
                g_avg1 = weighted_sum_channel(group1, 1, weight_g1)

                group2 = [ block[1][2], block[0][0], block[1][0], block[1][1]]
                weight_g2 = [0.25, 0.25, 0.25, 0.25]
                g_avg2 = weighted_sum_channel(group2, 1, weight_g2)
                # 判断是否是左边界，和是否同时是下边界
                if lef_bd:

                    pixels[x + 0, y + 1] = (0, g_avg2, 0)
                else:
                    pixels[x + 0, y + 1] = (0, g_avg1, 0)

                # 处理 (1,0): R数据
                group3 = [block[0][0], block[0][1], block[1][0], block[1][1]]

                r_avg = weighted_sum_channel(group3, 0, weight_r)
                pixels[x + 1, y + 0] = (r_avg, 0, 0)

                # 处理 (1,1): B数据
                b_avg = weighted_sum_channel(group3, 2, weight_b)
...
def main(input_directory, output_directory, r_factor, g_factor, b_factor):
    ...
    for filename in os.listdir(input_directory):
        ...
        with tempfile.NamedTemporaryFile(delete=False, suffix=".png") as temp_file1, \
            ...
            temp_path = temp_file1.name
            temp_path2 = temp_file2.name

        try:
            # 图像平滑
            smooth_image(input_path, temp_path2)
            print(f"我完成了图像处理")
            # 模拟图像缺陷
            process_image(temp_path2, temp_path)
...
if __name__ == "__main__":
    ...
    input_directory = r"F:\FPGA\unilumin_work\pic\pic_ku\kill_page"  # 输入文件夹路径
    output_directory = r"F:\FPGA\unilumin_work\pic\1219\t_1"  # 输出文件夹路径
    output_directory2 = r"F:\FPGA\unilumin_work\pic\1219\t_5"
    # 设定每个颜色通道的增益系数
    """
