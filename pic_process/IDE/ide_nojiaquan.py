#IDE:在整合亮度设置和模拟图像缺陷的基础上整合了缩放功能，和图像优化功能。
...
def main(input_directory, output_directory, r_factor, g_factor, b_factor):
    ...
    for filename in os.listdir(input_directory):
        ...
        try:
            # 图像平滑
            smooth_image(input_path, temp_path)
            # 处理图像
            process_image(temp_path, temp_path2)

            # 生成输出文件路径
            output_path = os.path.join(output_directory, filename)

            # 增加亮度
            increase_brightness(temp_path2, output_path, r_factor, g_factor, b_factor)
        finally:
            # 删除临时文件
...
def process_directory(input_dir, output_dir, width_scale=2, height_scale=3):
    ...
    for filename in os.listdir(input_dir):
        if filename.lower().endswith('.png'):
            ...
            output_filename = f"{name}_jz{ext}"
...
if __name__ == "__main__":
    ...
    output_directory = r"F:\FPGA\unilumin_work\pic\1219\fw3"  # 输出文件夹路径
    output_directory2 = r"F:\FPGA\unilumin_work\pic\1219"
    # 设定每个颜色通道的增益系数
    """
