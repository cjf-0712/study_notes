...
# 2025.0424 采用周围像素采样来优化画面，各颜色各四个模块，一个循环。
import tempfile
import os
    ...
    try:
        ...
        for y in range(0, height, 4):
            for x in range(0, width, 4):
                ...
                # 处理 (0,0): B1数据
                group1 = [block[0][0], block[1][0], block[1][1], block[0][1]]
                ...
                # 处理 (1,2): B2数据
                group4 = [block[1][2], block[0][2], block[0][3], block[1][3]]
                b_avg2 = weighted_sum_channel(group4, 2, weight1)
                pixels[x + 1, y + 2] = (0, 0, b_avg2)

                # 处理 (0,2): G2数据
                group5 = [block[0][2], block[1][2], block[0][3], block[1][3]]
                ...
                # 处理 (1,3): R2数据
                group6 = [block[1][3], block[1][2], block[2][2], block[2][3]]
                r_avg2 = weighted_sum_channel(group6, 0, weight1)
                pixels[x + 1, y + 3] = (r_avg2, 0, 0)

                # 处理 (2,0): R3数据
                group7 = [block[2][0], block[3][0], block[2][1], block[3][1]]
                r_avg3 = weighted_sum_channel(group7, 0, weight1)
                pixels[x + 2, y + 0] = (r_avg3, 0, 0)

                # 处理 (3,1): G3数据
                group8 = [block[3][1], block[3][0], block[2][0], block[2][1]]
                ...
                # 处理 (2,1): B3数据
                group9 = [block[2][1], block[3][1], block[2][0], block[3][0]]
                ...
                # 处理 (3,2): R4数据
                group11 = [block[3][2], block[2][2], block[2][3], block[3][3]]
...
if __name__ == "__main__":
    ...
    output_directory = r"F:\FPGA\unilumin_work\pic\pic_ku\spr4_0425_2"  # 输出文件夹路径
    # 设定每个颜色通道的增益系数
    """
