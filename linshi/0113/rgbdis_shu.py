        ...
        for y in range(height):
            for x in range(width):
                ...
                red_bbox = [
                    ...
                    (top_left_x +(scale_factor // 3) , top_left_y + scale_factor)
                ]
                draw.rectangle(red_bbox, fill=(r, 0, 0))

                # 绿色覆盖上面三分之一
                green_bbox = [
                    (top_left_x + ((scale_factor // 3)), top_left_y ),
                    (top_left_x + ((scale_factor // 3)*2), top_left_y + scale_factor)
                ]
                draw.rectangle(green_bbox, fill=(0, g, 0))

                # 蓝色覆盖下面三分之一
                blue_bbox = [
                    (top_left_x + ((scale_factor // 3)*2), top_left_y ),
...
if __name__ == "__main__":
    ...
    output_folder = r"F:\FPGA\unilumin_work\pic\0114\dis_shu_1"  # 输出文件夹路径
